...
class RiraTagLib {
    ...
    Closure paginate = { Map attrsMap ->
        ...
        if (!attrs.boolean('omitPrev')) {
            ...
            {
                writer << disabledLI(attrs.prev ?: messageSource.getMessage('paginate.prev', null, messageSource.getMessage('default.paginate.prev', null, '&laquo;', locale), locale))
            }
            else
            {
        ...
        if (steps && laststep > firststep) {
            ...
            (beginstep..endstep).each { int i ->
                if (currentstep == i) {
                    writer << callLink((Map)linkTagAttrs.clone(), true) {i.toString()}
//                    writer << "<span class=\"currentStep\">${i}</span>"
        ...
        if (!attrs.boolean('omitNext')) {
            if(currentstep >= laststep)
            {
                ...
                linkParams.offset = offset + max
                writer << callLink((Map) linkTagAttrs.clone()) {
                    (attrs.next ? attrs.next : messageSource.getMessage('paginate.next', null, messageSource.getMessage('default.paginate.next', null, '&raquo;', locale), locale))
                }
            }
        }
    }

    private callLinkPaginate(Map attrs, Object body) {
        GroovyPage.captureTagOutput(gspTagLibraryLookup, 'g', 'link', attrs, body, webRequest)
    }

    private callLink(Map attrs, boolean active=false, Object body) {
        StringBuffer tmp
        if( active )
            tmp = new StringBuffer( "<li class=active>" )
        else
            tmp = new StringBuffer( "<li>" )
        tmp.append callLinkPaginate(attrs, body)
        tmp.append "</li>"
        tmp
    }

    private disabledLI( msg ) {
        new StringBuffer("<li class=disabled><span>").append(msg).append("</span></li>")
    }
}
